...
// Cargar librería JsBarcode dinámicamente
if (!window.JsBarcode) {
...
<button type="button" id="btnPromociones" class="ant-btn css-1r8qqf1 ant-btn-default ant-btn-color-default ant-btn-variant-outlined ant-btn-sm mb-4" style="border-radius:6px; border:1px solid #28a745; background:#d4edda; color:#155724; font-size:0.9rem; padding:2px 12px; margin-left:20px;" onclick="mostrarModalPromocionesGenerales()">
        🏷️ Ver Promociones
      </button>
    ...
    try {
      ...
      promocionesHTML.push(`
        <div class="col-12 col-md-6 col-lg-4 mb-3">
          <div class="card h-100 border-success" style="cursor:pointer;" onclick="mostrarProductoPromocion('${producto.pr_sku}')">
  ...
  resultadoDiv.innerHTML = `
    <div class="mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="text-success">🏷️ Productos con Promociones</h4>
        <span class="badge bg-success text-white">${productosConPromociones.length} encontrados</span>
      </div>
...
function actualizarProgreso(porcentaje, mensaje, detalle = '') {
  ...
  const color = porcentaje >= 100 ? 'success' : porcentaje >= 50 ? 'success' : 'primary';
...
// btnPromociones usa onclick directamente para mejor compatibilidad móvil
renderUltimaActualizacion();
...
function renderHistorial() {
                ...
                ${historial
                  .map((producto) => {
                    ...
                    return `<li class="list-group-item d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
                      ...
                      <span class="ms-md-2">
                        <button class="btn btn-primary btn-sm consultar-historial-btn" title="Ver detalles completos" data-sku="${sku}">
  ...
  // Botones consultar del historial
  const consultarBtns = historialDiv.querySelectorAll('.consultar-historial-btn');
  consultarBtns.forEach(btn => {
    btn.addEventListener('click', function(e) {
      const sku = btn.getAttribute('data-sku');
      consultarProductoDesdeHistorial(sku);
    });
  });
}
...
function mostrarDetalleProducto(producto) {
  ...
  // Generar código de barras funcional
  setTimeout(() => {
    generarCodigoBarras(producto);
  }, 100);
...
// Función para generar código de barras funcional
function generarCodigoBarras(producto) {
  const codigoBarras = producto.pr_sku || producto.pr_gtin || '';
  const canvasId = `barcode-${producto.pr_sku}`;
  const canvas = document.getElementById(canvasId);
  
  if (!canvas || !codigoBarras) {
    console.warn('No se puede generar código de barras: canvas o código faltante');
    return;
  }
  ...
  const intentarGenerar = () => {
    if (window.JsBarcode) {
      try {
        // Determinar el formato del código de barras según la longitud
        let formato = 'CODE128';
        const codigo = String(codigoBarras);
        
        if (codigo.length === 13 && /^\d+$/.test(codigo)) {
          formato = 'EAN13';
        } else if (codigo.length === 8 && /^\d+$/.test(codigo)) {
          formato = 'EAN8';
        } else if (codigo.length === 12 && /^\d+$/.test(codigo)) {
          formato = 'UPC';
        }
        
        console.log(`Generando código de barras ${formato} para: ${codigo}`);
        
        JsBarcode(canvas, codigo, {
          format: formato,
          width: 2,
          height: 50,
          displayValue: false, // No mostrar el texto debajo (ya lo tenemos)
          margin: 0,
          background: "#ffffff",
          lineColor: "#000000"
        });
        ...
        ctx.fillText('Código no válido', canvas.width / 2, canvas.height / 2);
      }
    } else {
      // Reintentar en 100ms si JsBarcode no está listo
      setTimeout(intentarGenerar, 100);
    }
  };
  ...
  resultadoDiv.innerHTML = `
    <div class="row align-items-center justify-content-center g-0 mb-4" style="padding: 0px 10px;">
      ...
      <div class="col-12 col-lg-8 text-center">
        <div class="fw-bold nombre-producto-responsive" style="color:#616161;">${producto.pr_name}</div>
        <div class="d-flex justify-content-center align-items-center gap-3 flex-wrap">
          <div class="d-flex align-items-center gap-2">
            <div class="text-center">
              <div class="fw-bold text-danger precio-responsive">$ ${precioConIVA.toFixed(2)}</div>
              <div class="text-muted" style="font-size:0.8rem; margin-top:-2px;">CONTADO</div>
            </div>
            <div class="text-muted mx-2" style="font-size:1.5rem;">|</div>
            <div class="text-center">
              <div class="fw-bold text-primary precio-responsive">$ ${precioMSI.toFixed(2)}</div>
              <div class="text-muted" style="font-size:0.8rem; margin-top:-2px;">MSI</div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-3 mb-3">
          <div class="text-center">
            <canvas id="barcode-${producto.pr_sku}" class="barcode-canvas"></canvas>
          </div>
          <div class="text-center mt-2">
            <small class="fw-bold text-dark">${producto.pr_sku || producto.pr_gtin || 'N/A'}</small>
          </div>
        </div>
  ...
  if (promociones.length > 0) {
    ...
    if (promocionesOficiales.length > 0) {
      const promocionesHTML = promocionesOficiales.map(promo => {
        ...
        return `<span class="badge bg-success text-white me-2 mb-1" style="font-size:0.85rem;">🔥 ${textoPromo}</span>`;
      }).join('');
      ...
      infoPromocionesHTML = `
        <div class="mt-3 p-2" style="background-color:#f8f9fa; border-radius:8px;">
          ...
          <div class="text-center mt-2">
            <small class="text-success" style="font-weight:500;">⚠️ Las promociones solo aplican pagando de contado</small>
          </div>
        </div>
      `;
    } else {
...
// Función para mostrar promociones generales en modal
async function mostrarModalPromocionesGenerales() {
  ...
  const modalBody = document.getElementById('promociones-generales-modal-body');
  ...
  try {
    console.log('🎯 Consultando promociones generales...');
      ...
      modalBody.innerHTML = `
        <div class="alert alert-info">
          <div class="text-center">
            ...
            <h6 class="mt-3">ℹ️ Sin promociones</h6>
    ...
    promociones.forEach((promo, index) => {
      ...
      if (matchPrecio) {
        ...
        precioPromoTotal = Math.round(Number(matchPrecio[2]));
      } else {
        ...
        precioPromoTotal = Math.round(precioUnitarioPromo * cantidadPromo);
      }
      
      // Crear card para cada promoción
      html += `
        <div class="col-md-6 col-lg-4 mb-4">
          <div class="card h-100 border-success shadow-sm">
            <div class="card-header bg-success text-white text-center">
              <h6 class="card-title mb-0">🏷️ ${nombre}</h6>
            </div>
            <div class="card-body p-3">
              <div class="productos-lista">
                ${productosEnPromo.length > 0 ? 
                      ...
                      <div class="d-flex align-items-center mb-3 p-2 border rounded">
                        ${urlImg ? `
                          <div class="me-3" style="min-width: 60px;">
                            <img src="${urlImg}" alt="${producto.pr_name}" 
                                 style="width: 60px; height: 60px; object-fit: contain; border-radius: 8px; background: #f8f9fa;" 
                                 onerror="this.style.display='none'">
                          </div>
                        ` : `
                          <div class="me-3 d-flex align-items-center justify-content-center bg-light rounded" style="min-width: 60px; height: 60px;">
                            <i class="bi bi-image text-muted" style="font-size: 1.5rem;"></i>
                          </div>
                        `}
                        <div class="flex-grow-1">
                          <div class="fw-bold" style="font-size: 0.9rem; line-height: 1.2;">${producto.pr_name}</div>
                          <div class="text-success fw-bold mt-1">$${precioConIVA.toFixed(2)}</div>
                        </div>
                        <div class="ms-2">
                          <button class="btn btn-primary btn-sm" onclick="consultarProductoDesdeModal('${producto.pr_sku}')" title="Ver detalles completos">
                            <i class="bi bi-search"></i>
                          </button>
                        </div>
                      </div>
                  ...
                  `<div class="text-center text-muted p-3">
                    ...
                    <p class="mt-2 mb-0">Productos no disponibles</p>
                  </div>`
                }
              </div>
            </div>
    ...
    // Abrir modal con lógica específica para móviles
    if (isMobile || isIOS) {
      ...
      console.log('💻 Abriendo modal en desktop...');
    ...
    modalBody.innerHTML = `
      <div class="alert alert-danger">
        <div class="text-center">
          ...
          <h6 class="mt-3">❌ Error al cargar promociones</h6>
          <p>${error.message}</p>
          <button class="btn btn-success" onclick="mostrarModalPromocionesGenerales()">
            🔄 Reintentar
          </button>
        </div>
      </div>
...
// Función para cerrar modal en dispositivos móviles
function cerrarModalMobile() {
  ...
  // También intentar cerrar instancia Bootstrap si existe
  try {
    ...
    console.log('Bootstrap modal no activo');
  }
}

// Función para consultar producto desde el modal (simula escaneo)
async function consultarProductoDesdeModal(sku) {
  try {
    // Cerrar el modal
    const modal = bootstrap.Modal.getInstance(document.getElementById('promocionesGeneralesModal'));
    if (modal) {
      modal.hide();
    }
    ...
    const producto = productos.find(p => 
      String(p.pr_sku) === String(sku) || 
      String(p.pr_gtin) === String(sku)
    );
    
    if (producto) {
      // Limpiar el input y mostrar el producto
      inputCodigo.value = '';
      
      // Mostrar el detalle del producto (igual que si lo hubiera escaneado)
      mostrarDetalleProducto(producto);
      ...
      resultadoDiv.innerHTML = `
        <div class="alert alert-danger" role="alert">
          ...
          <p>No se pudo encontrar el producto con SKU: ${sku}</p>
        </div>
      `;
      setTimeout(() => {
        resultadoDiv.innerHTML = "";
    ...
    console.error('Error consultando producto desde modal:', error);
    resultadoDiv.innerHTML = `
      <div class="alert alert-danger" role="alert">
        <h6>❌ Error</h6>
        <p>Error al consultar el producto: ${error.message}</p>
      </div>
    `;
    setTimeout(() => {
      resultadoDiv.innerHTML = "";
...
// Función para consultar producto desde el historial
async function consultarProductoDesdeHistorial(sku) {
  try {
    // Buscar el producto
    const productos = await getAllProductos();
    const producto = productos.find(p => 
    ...
    if (producto) {
      ...
      // NO lo agregamos de nuevo al historial para evitar duplicados
      // Solo actualizamos la fecha del producto existente
      const indiceExistente = historial.findIndex(h => 
        (h.pr_sku && String(h.pr_sku) === String(sku)) || 
        (h.pr_gtin && String(h.pr_gtin) === String(sku))
      );
      
      if (indiceExistente !== -1) {
        // Mover el producto al inicio del historial con nueva fecha
        const productoExistente = historial.splice(indiceExistente, 1)[0];
        productoExistente.FECHA_ESCANEO = new Date().toISOString();
        historial.unshift(productoExistente);
        setCookie("historial", JSON.stringify(historial), 30);
        renderHistorial();
      }
    ...
    console.error('Error consultando producto desde historial:', error);
    resultadoDiv.innerHTML = `
      <div class="alert alert-danger" role="alert">
        <h6>❌ Error</h6>
        <p>Error al consultar el producto: ${error.message}</p>
...
window.mostrarModalPromocionesGenerales = mostrarModalPromocionesGenerales;
...
style.innerHTML = `
  ...
  /* Estilos responsivos para nombre del producto */
  ...
  /* Estilos para código de barras funcional */
  .barcode-canvas {
    max-width: 100%;
    height: auto;
...
// Agregar estilos CSS específicos para móviles
